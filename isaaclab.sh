...
# Workaround for Warp CUDA UUID driver entrypoint issues
# Ensures you don't need to export this manually on every run
if [ -z "${WARP_DISABLE_CUDA_UUID+x}" ]; then
    export WARP_DISABLE_CUDA_UUID=1
fi
